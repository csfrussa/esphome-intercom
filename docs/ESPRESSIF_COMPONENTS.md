...
| `gmf_audio` / `aud_rate_cvt` | GMF audio pipeline elements, including rate conversion, interleave and deinterleave | Defer, strong candidate | Official examples run codec at 48 kHz and insert `aud_rate_cvt` before AEC. This matches our 48 kHz bus to 16 kHz processor shape, but adopting it means moving toward GMF pipeline scheduling. |
| `esp_audio_effects` / `esp_ae_rate_cvt` | Standalone C API behind GMF rate conversion | Defer, likely next migration target | This can replace the custom FIR decimator with Espressif's rate converter without importing the whole GMF pipeline. It must be isolated because it can change reference latency and phase, which are AEC-critical. |
