...
static const char *const TAG = "esphome_voip_stack.settings";
...
void ESPHomeVoipStack::load_settings_() {
  this->settings_pref_ = global_preferences->make_preference<StoredSettings>(fnv1_hash("esphome_voip_stack_settings"));

  StoredSettings stored;
  if (this->settings_pref_.load(&stored) && stored.version == SETTINGS_VERSION) {
    this->suppress_save_ = true;

    // Volume only when esphome_voip_stack owns the master_volume number;
...
void ESPHomeVoipStack::schedule_save_settings_() {
...
void ESPHomeVoipStack::save_settings_() {
  StoredSettings stored;
...
void ESPHomeVoipStack::set_volume(float volume) {
...
void ESPHomeVoipStack::set_auto_answer(bool enabled) {
...
void ESPHomeVoipStack::set_do_not_disturb(bool enabled) {
  this->do_not_disturb_ = enabled;
  ESP_LOGI(TAG, "Do-not-disturb set to %s", enabled ? "ON" : "OFF");
  // Persisted via switch restore_mode, not save_settings_.
}

void ESPHomeVoipStack::set_mic_gain_db(float db) {
...
void ESPHomeVoipStack::add_contact(const std::string &entry) {
...
void ESPHomeVoipStack::remove_contact(const std::string &name) {
  const std::string before = this->phonebook_.current_name();
...
void ESPHomeVoipStack::set_contacts(const std::string &contacts_csv) {
  this->phonebook_.set_self_name(this->device_name_);
...
std::string ESPHomeVoipStack::normalize_phonebook_for_transport_(const std::string &contacts_csv) {
...
std::string ESPHomeVoipStack::normalize_roster_json_for_transport_(const std::string &roster_json) {
...
bool ESPHomeVoipStack::apply_roster_json_contacts_(const std::string &roster_json) {
...
void ESPHomeVoipStack::update_contacts() {
...
void ESPHomeVoipStack::track_csv_(const std::string &csv) {
...
void ESPHomeVoipStack::commit_cycle_() {
...
bool ESPHomeVoipStack::maybe_auto_select_ha_first_() {
...
void ESPHomeVoipStack::flush_contacts() {
...
bool ESPHomeVoipStack::set_contact(const std::string &name) {
...
void ESPHomeVoipStack::call_contact(const std::string &name) {
...
void ESPHomeVoipStack::next_contact() {
...
void ESPHomeVoipStack::prev_contact() {
  if (this->phonebook_.empty()) return;
...
const std::string &ESPHomeVoipStack::get_current_destination() const {
  return this->phonebook_.current_name();
}

const std::string &ESPHomeVoipStack::get_current_contact_ip() const {
  return this->phonebook_.current_ip();
}

uint16_t ESPHomeVoipStack::get_current_contact_port() const {
  return this->phonebook_.current_port();
}

uint16_t ESPHomeVoipStack::get_current_contact_rtp_port() const {
  const auto *c = this->phonebook_.current();
  return c ? c->rtp_port : 0;
}

bool ESPHomeVoipStack::get_current_contact_sip_transport_tcp() const {
  const auto *c = this->phonebook_.current();
  return c != nullptr && c->endpoint_kind == ContactEndpointKind::SIP && c->sip_transport_tcp;
}

void ESPHomeVoipStack::publish_destination_() {
...
void ESPHomeVoipStack::publish_caller_(const std::string &caller_name) {
...
void ESPHomeVoipStack::publish_contacts_() {
...
std::string ESPHomeVoipStack::get_contacts_csv() const {
