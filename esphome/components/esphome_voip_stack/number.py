...
ESPHomeVoipStackVolume = esphome_voip_stack_ns.class_(
    "ESPHomeVoipStackVolume", number.Number, cg.Parented.template(ESPHomeVoipStack)
)
ESPHomeVoipStackMicGain = esphome_voip_stack_ns.class_(
    "ESPHomeVoipStackMicGain", number.Number, cg.Parented.template(ESPHomeVoipStack)
)
...
CONFIG_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_MASTER_VOLUME): _number_schema(
            ESPHomeVoipStackVolume, "mdi:volume-high", 0, 100, 5
        ),
        # Mic gain in dB (-20 to +20), default 0dB
        cv.Optional(CONF_MIC_GAIN): _number_schema(
            ESPHomeVoipStackMicGain, "mdi:microphone", -20, 20, 1
        ),
    }
)
