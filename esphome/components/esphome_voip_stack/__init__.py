...
CONF_ESPHOME_VOIP_STACK_ID = "esphome_voip_stack_id"
...
# subscription package binds that HA text_sensor here; esphome_voip_stack then
...
esphome_voip_stack_ns = cg.esphome_ns.namespace("esphome_voip_stack")
ESPHomeVoipStack = esphome_voip_stack_ns.class_("ESPHomeVoipStack", cg.Component)
TransportType = esphome_voip_stack_ns.enum("TransportType", is_class=True)
PcmFormat = esphome_voip_stack_ns.enum("PcmFormat", is_class=True)
...
def _validate_voip_audio_config(value):
    for primary, extra_key in (
        ...
        if 1 + len(value[extra_key]) > 8:
            raise cv.Invalid(
                f"esphome_voip_stack.audio.{extra_key} supports at most 7 extra formats "
...
def _validate_tx_reframe_formats(tx_fmt: dict, tx_formats: list[dict]) -> None:
    for fmt in tx_formats:
        if _has_auto_audio_field(fmt):
            raise cv.Invalid(
                "esphome_voip_stack.audio.tx_formats entries must be explicit. "
        ...
        for key in (CONF_SAMPLE_RATE, CONF_PCM_FORMAT, CONF_CHANNELS):
            if fmt[key] != tx_fmt[key]:
                raise cv.Invalid(
                    "esphome_voip_stack.audio.tx_formats can only declare packet-time reframes "
...
def _pcm_from_bits(bits: int) -> str:
    ...
    if bits == 32:
        ...
        return "s24le_in_s32"
    raise cv.Invalid(f"esphome_voip_stack audio auto cannot map {bits} bits per sample to a PCM format")
...
def _resolve_audio_format(config: dict, direction: str, value) -> dict:
    ...
    if _is_auto(value):
        ...
        raise cv.Invalid(
            f"esphome_voip_stack.audio.{direction}: auto could not derive the PCM format from the referenced "
    ...
    if any(_is_auto(resolved[key]) for key in (CONF_SAMPLE_RATE, CONF_PCM_FORMAT, CONF_CHANNELS, CONF_FRAME_MS)):
        ...
        if derived is None:
            source = "microphone/source" if direction == CONF_TX else "speaker"
            raise cv.Invalid(
                f"esphome_voip_stack.audio.{direction}: one or more fields are auto, but the referenced {source} "
...
# === Action classes (for YAML: esphome_voip_stack.next_contact, etc.) ===
NextContactAction = esphome_voip_stack_ns.class_("NextContactAction", automation.Action)
PrevContactAction = esphome_voip_stack_ns.class_("PrevContactAction", automation.Action)
StartAction = esphome_voip_stack_ns.class_("StartAction", automation.Action)
StopAction = esphome_voip_stack_ns.class_("StopAction", automation.Action)
AnswerCallAction = esphome_voip_stack_ns.class_("AnswerCallAction", automation.Action)
DeclineCallAction = esphome_voip_stack_ns.class_("DeclineCallAction", automation.Action)
CallToggleAction = esphome_voip_stack_ns.class_("CallToggleAction", automation.Action)
PublishEntityStatesAction = esphome_voip_stack_ns.class_("PublishEntityStatesAction", automation.Action)

# Parameterized actions
SetVolumeAction = esphome_voip_stack_ns.class_("SetVolumeAction", automation.Action)
SetMicGainDbAction = esphome_voip_stack_ns.class_("SetMicGainDbAction", automation.Action)
SetContactsAction = esphome_voip_stack_ns.class_("SetContactsAction", automation.Action)
SetContactAction = esphome_voip_stack_ns.class_("SetContactAction", automation.Action)
CallContactAction = esphome_voip_stack_ns.class_("CallContactAction", automation.Action)
SetRemoteEndpointAction = esphome_voip_stack_ns.class_("SetRemoteEndpointAction", automation.Action)
AddContactAction = esphome_voip_stack_ns.class_("AddContactAction", automation.Action)
RemoveContactAction = esphome_voip_stack_ns.class_("RemoveContactAction", automation.Action)
FlushContactsAction = esphome_voip_stack_ns.class_("FlushContactsAction", automation.Action)
UpdateContactsAction = esphome_voip_stack_ns.class_("UpdateContactsAction", automation.Action)
SetHaPeerNameAction = esphome_voip_stack_ns.class_("SetHaPeerNameAction", automation.Action)

# === Condition classes (for YAML: esphome_voip_stack.is_idle, etc.) ===
VoipIsIdleCondition = esphome_voip_stack_ns.class_("VoipIsIdleCondition", automation.Condition)
VoipIsRingingCondition = esphome_voip_stack_ns.class_("VoipIsRingingCondition", automation.Condition)
VoipIsInCallCondition = esphome_voip_stack_ns.class_("VoipIsInCallCondition", automation.Condition)
VoipIsCallingCondition = esphome_voip_stack_ns.class_("VoipIsCallingCondition", automation.Condition)
VoipIsIncomingCondition = esphome_voip_stack_ns.class_("VoipIsIncomingCondition", automation.Condition)
VoipDestinationIsCondition = esphome_voip_stack_ns.class_("VoipDestinationIsCondition", automation.Condition)
VoipIsHaDestinationCondition = esphome_voip_stack_ns.class_("VoipIsHaDestinationCondition", automation.Condition)
...
ESPHomeVoipStackAutoAnswerCls = esphome_voip_stack_ns.class_(
    "ESPHomeVoipStackAutoAnswer", _switch_ns.Switch, cg.Parented.template(ESPHomeVoipStack)
)
ESPHomeVoipStackDndSwitchCls = esphome_voip_stack_ns.class_(
    "ESPHomeVoipStackDndSwitch", _switch_ns.Switch, cg.Parented.template(ESPHomeVoipStack)
)
ESPHomeVoipStackVolumeCls = esphome_voip_stack_ns.class_(
    "ESPHomeVoipStackVolume", _number_ns.Number, cg.Parented.template(ESPHomeVoipStack)
)
ESPHomeVoipStackMicGainCls = esphome_voip_stack_ns.class_(
    "ESPHomeVoipStackMicGain", _number_ns.Number, cg.Parented.template(ESPHomeVoipStack)
)
...
CONFIG_SCHEMA = cv.Schema(
    {
        cv.GenerateID(): cv.declare_id(ESPHomeVoipStack),
        ...
        # bit-depth, or integer gain conversion before esphome_voip_stack sees them.
        ...
        # `switch:`/`number: - platform: esphome_voip_stack`.
        ...
        cv.Optional(CONF_PROCESSOR_ID): cv.invalid(
            "esphome_voip_stack.processor_id was removed. Use native ESPHome "
        ...
        cv.Optional(CONF_AEC_REF_DELAY_MS): cv.invalid(
            "esphome_voip_stack.aec_reference_delay_ms was removed with standalone "
            "VoIP AEC. Configure AEC on esp_audio_stack instead."
        ),
        # Place VoIP staging buffers in PSRAM. This applies only to buffers
        # still owned by esphome_voip_stack: the optional mic ring, TX chunk, and mic
...
def _consume_voip_sockets(config):
    """Reserve lwIP sockets for esphome_voip_stack at validation time.
    ...
    socket.consume_sockets(2, "esphome_voip_stack_sip", socket.SocketType.UDP)(config)
    socket.consume_sockets(2, "esphome_voip_stack_sip_tcp")(config)
    socket.consume_sockets(1, "esphome_voip_stack_sip", socket.SocketType.TCP_LISTEN)(config)
    extra = config.get(CONF_NETWORK_SOCKET_HEADROOM, 0)
    if extra:
        socket.consume_sockets(extra, "esphome_voip_stack_headroom")(config)
    return config
...
def _final_validate(config):
    ...
    if CONF_MICROPHONE in config and CONF_MICROPHONE_SOURCE in config:
        raise cv.Invalid(
            "Use only one of esphome_voip_stack.microphone or esphome_voip_stack.microphone_source."
        )
    ...
    if protocol in (TRANSPORT_UDP, TRANSPORT_TCP):
        ...
        for direction, fmt in checks:
            ...
            if payload_bytes > max_payload:
                raise cv.Invalid(
                    f"esphome_voip_stack RTP audio.{direction} payload is {payload_bytes} bytes, "
    ...
    if CONF_MICROPHONE in config:
        try:
            ...
            _LOGGER.warning(
                "esphome_voip_stack could not validate the referenced microphone audio "
                ...
                "esphome_voip_stack.audio.tx format."
            )

    if CONF_MICROPHONE_SOURCE in config:
        microphone.final_validate_microphone_source_schema(
            "esphome_voip_stack", sample_rate=tx_fmt[CONF_SAMPLE_RATE]
    ...
    if CONF_SPEAKER in config:
        try:
            ...
            _LOGGER.warning(
                "esphome_voip_stack speaker format was not declared by the referenced "
                "speaker component, so ESPHome cannot check it at compile time. "
                "Continuing with the explicitly declared esphome_voip_stack.audio.rx format."
            )
        ...
        if config.get(CONF_DC_OFFSET_REMOVAL, False):
            for audio_stack in (audio_stack_configs if isinstance(audio_stack_configs, list) else [audio_stack_configs]):
                if isinstance(audio_stack, dict) and audio_stack.get("correct_dc_offset", False):
                    raise cv.Invalid(
                        "Both esphome_voip_stack.dc_offset_removal and esp_audio_stack.correct_dc_offset are enabled. "
...
async def _build_voip_text_sensors(var, config):
    ...
    # "tcp") so the HA-side homeassistant_voip_stack integration can route calls from a
    # self-describing `esphome_voip_stack:` declaration.
    ...
    # Last terminal reason: required by homeassistant_voip_stack/card mirror mode.
...
async def _build_voip_auto_entities(var, config):
    ...
    # that already declare `switch:`/`number: - platform: esphome_voip_stack`
    ...
    if config[CONF_AUTO_ENTITIES]:
        ...
        aa_id = cv.declare_id(ESPHomeVoipStackAutoAnswerCls)(f"{config[CONF_ID].id}_auto_answer")
        ...
        dnd_id = cv.declare_id(ESPHomeVoipStackDndSwitchCls)(f"{config[CONF_ID].id}_dnd")
        ...
        if CONF_SPEAKER in config:
            vol_id = cv.declare_id(ESPHomeVoipStackVolumeCls)(f"{config[CONF_ID].id}_volume")
        ...
        if CONF_MICROPHONE in config or CONF_MICROPHONE_SOURCE in config:
            mg_id = cv.declare_id(ESPHomeVoipStackMicGainCls)(f"{config[CONF_ID].id}_mic_gain")
...
def _register_simple_action(name, action_class):
    """Register a parameter-less Parented<ESPHomeVoipStack> action.

    The codegen all the simple actions need is identical:
        var = new_Pvariable; parent = use_id(esphome_voip_stack); var.set_parent(parent)
...
_register_simple_action("esphome_voip_stack.next_contact", NextContactAction)
_register_simple_action("esphome_voip_stack.prev_contact", PrevContactAction)
_register_simple_action("esphome_voip_stack.start", StartAction)
_register_simple_action("esphome_voip_stack.stop", StopAction)
_register_simple_action("esphome_voip_stack.answer_call", AnswerCallAction)
...
_register_simple_action("esphome_voip_stack.call_toggle", CallToggleAction)
_register_simple_action("esphome_voip_stack.publish_entity_states", PublishEntityStatesAction)
...
_register_templated_action(
    "esphome_voip_stack.set_mic_gain_db",
    SetMicGainDbAction,
    CONF_GAIN_DB,
...
_register_templated_action(
    "esphome_voip_stack.set_contact",
    SetContactAction,
    CONF_CONTACT,
    cv.string,
    cg.std_string,
...
@automation.register_action(
    "esphome_voip_stack.set_remote_endpoint",
...
def _validate_add_contact_action(config):
    ...
    if CONF_NAME not in config:
        raise cv.Invalid("esphome_voip_stack.add_contact requires either entry or name")
    return config
...
for _add_contact_action_name in ("esphome_voip_stack.add_contact", "esphome_voip_stack.add_contacts"):
    automation.register_action(
...
_register_simple_action("esphome_voip_stack.flush_contacts", FlushContactsAction)
_register_simple_action("esphome_voip_stack.update_contacts", UpdateContactsAction)
...
_register_simple_condition("esphome_voip_stack.is_idle", VoipIsIdleCondition)
_register_simple_condition("esphome_voip_stack.is_ringing", VoipIsRingingCondition)
_register_simple_condition("esphome_voip_stack.is_in_call", VoipIsInCallCondition)
_register_simple_condition("esphome_voip_stack.is_calling", VoipIsCallingCondition)
_register_simple_condition("esphome_voip_stack.is_incoming", VoipIsIncomingCondition)


CONF_DESTINATION = "destination"

@automation.register_condition(
    "esphome_voip_stack.destination_is",
...
_register_simple_condition("esphome_voip_stack.is_ha_destination", VoipIsHaDestinationCondition)
