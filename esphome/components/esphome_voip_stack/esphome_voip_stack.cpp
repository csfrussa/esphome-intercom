...
static const char *const TAG = "esphome_voip_stack";

void ESPHomeVoipStack::append_audio_format_(AudioFormatList *list, const AudioFormat &format) {
...
bool ESPHomeVoipStack::ensure_mic_processing_buffer_() {
#ifdef USE_ESPHOME_VOIP_STACK_MIC
  if (this->tx_audio_format_.pcm_format != PcmFormat::S16LE) {
    ESP_LOGE(TAG, "mic_gain and dc_offset_removal require esphome_voip_stack.audio.tx.pcm_format: s16le");
    return false;
  }
...
void ESPHomeVoipStack::cleanup_partial_setup_() {
  ...
  audio_processor::force_delete_pinned_task(&this->tx_task_handle_, &this->tx_task_stack_,
                                             ESPHomeVoipStack::kTxTaskStackBytes);
...
bool ESPHomeVoipStack::allocate_setup_buffers_() {
#ifdef USE_ESPHOME_VOIP_STACK_MIC
  ...
  if (this->has_microphone_() && this->dc_offset_removal_) {
    if (this->tx_audio_format_.pcm_format != PcmFormat::S16LE) {
      ESP_LOGE(TAG, "dc_offset_removal requires esphome_voip_stack.audio.tx.pcm_format: s16le");
      return false;
    }
...
bool ESPHomeVoipStack::setup_audio_processor_() {
#ifdef USE_ESPHOME_VOIP_STACK_MIC
...
bool ESPHomeVoipStack::setup_transport_() {
#ifdef USE_ESPHOME_VOIP_SIP_TRANSPORT
  ...
  this->transport_->set_audio_callback(ESPHomeVoipStack::transport_audio_callback_, this);
  this->transport_->set_sip_signal_callback(ESPHomeVoipStack::transport_sip_signal_callback_, this);
  this->transport_->set_connection_callback(ESPHomeVoipStack::transport_connection_callback_, this);
  this->transport_->set_accept_callback(ESPHomeVoipStack::transport_accept_callback_, this);
...
void ESPHomeVoipStack::transport_audio_callback_(void *ctx, const uint8_t *pcm, size_t bytes) {
  static_cast<ESPHomeVoipStack *>(ctx)->on_audio_received_(pcm, bytes);
}

void ESPHomeVoipStack::transport_sip_signal_callback_(void *ctx, const SipSignal &signal) {
  static_cast<ESPHomeVoipStack *>(ctx)->on_sip_signal_received_(signal);
}

void ESPHomeVoipStack::transport_connection_callback_(void *ctx, bool connected) {
  static_cast<ESPHomeVoipStack *>(ctx)->on_connection_change_(connected);
}

bool ESPHomeVoipStack::transport_accept_callback_(void *ctx) {
  return static_cast<ESPHomeVoipStack *>(ctx)->can_accept_session_();
}

bool ESPHomeVoipStack::start_runtime_tasks_() {
#ifdef USE_ESPHOME_VOIP_STACK_MIC
  ...
  if (this->has_microphone_()) {
    if (!audio_processor::start_pinned_task(ESPHomeVoipStack::tx_task, "voip_tx",
                                             ESPHomeVoipStack::kTxTaskStackBytes, this, 5, 0,
...
void ESPHomeVoipStack::publish_initial_state_later_() {
...
void ESPHomeVoipStack::fail_setup_() {
  this->cleanup_partial_setup_();
  this->mark_failed();
}

void ESPHomeVoipStack::setup() {
  ESP_LOGI(TAG, "Setting up ESPHome VoIP Stack...");
  ...
  esp_event_handler_instance_register(IP_EVENT, ESP_EVENT_ANY_ID,
                                      &ESPHomeVoipStack::ip_event_handler_,
                                      this, nullptr);
  this->publish_initial_state_later_();

  ESP_LOGI(TAG, "ESPHome VoIP Stack ready as SIP phone on %s/%u RTP UDP/%u",
           this->protocol_ == TransportType::TCP ? "TCP" : "UDP",
           (unsigned) this->sip_port_, (unsigned) this->rtp_port_);
}

void ESPHomeVoipStack::handle_call_timeouts_(uint32_t now_ms, uint32_t calling_timeout_ms) {
...
void ESPHomeVoipStack::loop() {
  ...
  if (this->cycle_active_ &&
    ...
    ESP_LOGD("esphome_voip_stack", "Phonebook update cycle auto-commit after %u ms",
...
void ESPHomeVoipStack::fire_timeout_decline_() {
...
void ESPHomeVoipStack::dump_config() {
  ESP_LOGCONFIG(TAG, "ESPHome VoIP Stack:");
  if (this->transport_) {
...
void ESPHomeVoipStack::set_remote_endpoint(const std::string &ip, uint16_t port, uint16_t rtp_port) {
  if (this->transport_ != nullptr) {
...
void ESPHomeVoipStack::set_remote_sip_transport_tcp(bool tcp) {
  if (this->transport_ != nullptr) {
...
std::string ESPHomeVoipStack::local_ip_string_() const {
...
std::string ESPHomeVoipStack::build_endpoint_string_() const {
  ...
  auto format_list_token = [&](const AudioFormatList &list) -> std::string {
    ...
    for (uint8_t i = 0; i < list.count; i++) {
      if (!out.empty()) out += ";";
      out += ESPHomeVoipStack::audio_format_token_(list.formats[i]);
    }
    return out;
  };
...
std::string ESPHomeVoipStack::build_sip_snapshot_string_() const {
  ...
  std::string selected_tx = ESPHomeVoipStack::audio_format_token_(this->current_tx_audio_format_);
  std::string selected_rx = ESPHomeVoipStack::audio_format_token_(this->current_rx_audio_format_);
  if (!this->last_terminal_call_id_.empty() && !this->last_reason_.empty() &&
      this->call_state_.load(std::memory_order_acquire) == CallState::IDLE) {
    selected_tx = ESPHomeVoipStack::audio_format_token_(this->last_terminal_tx_audio_format_);
    selected_rx = ESPHomeVoipStack::audio_format_token_(this->last_terminal_rx_audio_format_);
  }
  if (this->transport_ != nullptr) {
    ...
    if (this->call_state_.load(std::memory_order_acquire) != CallState::IDLE || this->last_reason_.empty()) {
      selected_tx = ESPHomeVoipStack::audio_format_token_(snap.selected_tx_format);
      selected_rx = ESPHomeVoipStack::audio_format_token_(snap.selected_rx_format);
    }
  }
  char out[256];
...
void ESPHomeVoipStack::publish_sip_snapshot_() {
...
void ESPHomeVoipStack::publish_endpoint_() {
...
void ESPHomeVoipStack::request_endpoint_publish_() {
  this->endpoint_publish_requested_.store(true, std::memory_order_release);
  this->enable_loop_soon_any_context();
}

void ESPHomeVoipStack::ip_event_handler_(void *arg, esp_event_base_t event_base,
                                    int32_t event_id, void *event_data) {
  if (event_base != IP_EVENT) return;
  if (event_id != IP_EVENT_STA_GOT_IP && event_id != IP_EVENT_ETH_GOT_IP) return;
  static_cast<ESPHomeVoipStack *>(arg)->request_endpoint_publish_();
}

// Wired from YAML via `api.on_client_connected:`.
void ESPHomeVoipStack::publish_entity_states() {
  // Re-publish on every HA reconnect so homeassistant_voip_stack sees it without
