...
`esphome_voip_stack` is the ESP SIP phone component. It owns SIP signaling, SDP
...
- **Standalone native ESPHome audio**: bind `esphome_voip_stack` directly to standard
...
`esphome_voip_stack` now defines its own local compile flags from YAML:
...
to `esphome_voip_stack`; maintained profiles use `16000:s16le:1:16`. Native ESPHome
...
Standalone `esphome_voip_stack` AEC has been removed.

Removed options:

| Removed | Replacement |
|---|---|
| `esphome_voip_stack.processor_id` | Configure `processor_id` on `esp_audio_stack`. |
| `esphome_voip_stack.aec_reference_delay_ms` | Configure AEC/AFE reference buffering on `esp_audio_stack`. |
| `switch: platform: esphome_voip_stack, aec:` | Use `esp_audio_stack`, `esp_aec` or `esp_afe` controls. |

Reason: `esphome_voip_stack` is not the right owner for I2S timing, speaker reference
...
Contacts can be declared directly in `esphome_voip_stack` for installs that do not
...
transport (`transport: udp` or `transport: tcp` on `esphome_voip_stack`). A name-only
...
`esphome_voip_stack.add_contact`, `esphome_voip_stack.add_contacts`,
`esphome_voip_stack.remove_contact`, `esphome_voip_stack.set_contacts` and
`esphome_voip_stack.flush_contacts`. Use those for custom local behavior only; HA
central contacts should be managed with `homeassistant_voip_stack.phonebook_*` services.
...
and services by `homeassistant_voip_stack`.
...
That profile intentionally avoids `esp_audio_stack` and binds `esphome_voip_stack` to
