...
static const char *const TAG = "esphome_voip_stack.fsm";
...
void ESPHomeVoipStack::set_call_identity_(const std::string &call_id,
...
void ESPHomeVoipStack::clear_call_identity_() {
  LockGuard l(this->call_state_mutex_);
...
ESPHomeVoipStack::CallSnapshot ESPHomeVoipStack::snapshot_call_identity_() const {
  LockGuard l(this->call_state_mutex_);
...
std::string ESPHomeVoipStack::get_current_call_id_() const {
  LockGuard l(this->call_state_mutex_);
  return this->current_call_id_;
}

void ESPHomeVoipStack::set_terminal_response_(const std::string &call_id, const std::string &reason) {
  LockGuard l(this->call_state_mutex_);
...
void ESPHomeVoipStack::clear_terminal_response_() {
  LockGuard l(this->call_state_mutex_);
...
void ESPHomeVoipStack::snapshot_terminal_response_(std::string *call_id, std::string *reason, uint32_t *age_ms) const {
  LockGuard l(this->call_state_mutex_);
...
bool ESPHomeVoipStack::recent_terminal_call_(const std::string &call_id, std::string *reason) const {
  if (call_id.empty()) {
    return false;
  }
...
bool ESPHomeVoipStack::send_sip_ringing_(const std::string &call_id) {
  return this->transport_ != nullptr && this->transport_->send_ringing(call_id);
}

bool ESPHomeVoipStack::send_sip_bye_(const std::string &call_id) {
  return this->transport_ != nullptr && this->transport_->send_bye(call_id);
}

bool ESPHomeVoipStack::send_sip_cancel_(const std::string &call_id) {
  return this->transport_ != nullptr && this->transport_->send_cancel(call_id);
}

bool ESPHomeVoipStack::send_sip_final_response_(const std::string &call_id, const std::string &reason) {
...
bool ESPHomeVoipStack::send_sip_answer_(const std::string &call_id) {
...
bool ESPHomeVoipStack::send_sip_invite_(const std::string &call_id,
                                   const std::string &caller_route,
                                   const std::string &caller_name,
                                   const std::string &dest_route,
                                   const std::string &dest_name) {
...
bool ESPHomeVoipStack::ignore_if_idle_or_stale_(const char *message_name,
...
void ESPHomeVoipStack::answer_call() {
  if (!this->is_ringing()) {
...
void ESPHomeVoipStack::decline_call(const std::string &reason) {
...
void ESPHomeVoipStack::call_toggle() {
  if (this->is_ringing()) {
    this->answer_call();
...
const char *ESPHomeVoipStack::get_state_str() const {
  return call_state_to_str(this->call_state_.load(std::memory_order_acquire));
}

void ESPHomeVoipStack::publish_state_() {
...
void ESPHomeVoipStack::clear_terminal_call_snapshot_() {
...
void ESPHomeVoipStack::publish_last_reason_(const std::string &reason) {
...
void ESPHomeVoipStack::set_active_(bool on) {
...
void ESPHomeVoipStack::set_in_call_(bool on) {
  if (on) {
    const std::string call_id = this->get_current_call_id_();
...
void ESPHomeVoipStack::notify_audio_tasks_() {
#ifdef USE_ESPHOME_VOIP_STACK_MIC
...
void ESPHomeVoipStack::set_call_state_(CallState new_state) {
...
void ESPHomeVoipStack::end_call_(CallEndReason reason, const std::string &detail) {
...
void ESPHomeVoipStack::on_audio_received_(const uint8_t *pcm, size_t bytes) {
...
void ESPHomeVoipStack::on_sip_signal_received_(const SipSignal &msg) {
...
void ESPHomeVoipStack::on_connection_change_(bool connected) {
  if (connected) {
...
bool ESPHomeVoipStack::can_accept_session_() const {
