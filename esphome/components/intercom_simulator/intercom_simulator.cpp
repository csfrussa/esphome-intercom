...
void IntercomSimulator::on_shutdown() {
  this->running_ = false;
...
std::string IntercomSimulator::handle_request_(const std::string &line) {
...
std::string IntercomSimulator::dispatch_(const std::string &method, const std::string &params) {
...
std::string IntercomSimulator::snapshot_json_() const {
  std::lock_guard<std::mutex> lock(this->mutex_);
  std::ostringstream out;
  out << "{";
...
void IntercomSimulator::write_framebuffer_() const {
...
void IntercomSimulator::write_audio_marker_(const std::string &label) const {
  ensure_parent_dir(this->speaker_output_path_);
  std::ofstream out(this->speaker_output_path_, std::ios::binary | std::ios::app);
  out << "intercom_simulator:" << label << "\n";
}

void IntercomSimulator::press_button_(const std::string &button) {
  std::lock_guard<std::mutex> lock(this->mutex_);
  if (button == "call") {
...
void IntercomSimulator::inject_event_(const std::string &params) {
  ...
  if (type == "set_option") {
    ...
    this->state_.intercom_state = "idle";
    this->state_.led_effect.clear();
...
void IntercomSimulator::ha_call_(const std::string &target, const std::string &caller) {
...
void IntercomSimulator::sip_invite_(const std::string &caller, const std::string &callee, const std::string &call_id) {
  if (callee == "Casa") {
    this->state_.softphone.state = "ringing";
    ...
    this->state_.intercom_state = "ringing";
