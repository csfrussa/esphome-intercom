"""Home Assistant VoIP Stack integration for Home Assistant.
...
def _store_manual_roster_entries(hass: HomeAssistant, entries) -> None:
    ...
    entry = _config_entry(hass)
    if entry is None:
        raise ConfigEntryError("Home Assistant VoIP Stack config entry is required for phonebook contacts")
...
def _update_sip_accounts(hass: HomeAssistant, accounts: list[dict]) -> None:
    entry = _config_entry(hass)
    if entry is None:
        raise ConfigEntryError("Home Assistant VoIP Stack config entry is required for SIP accounts")
    data = dict(entry.data)
...
def _register_esp_state_event_bridge(hass: HomeAssistant) -> None:
    """Forward ESP voip_state entity changes to homeassistant_voip_stack.call_event."""
    bucket = hass.data.setdefault(DOMAIN, {})
...
async def _press_device_button(hass: HomeAssistant, device: dict, key: str, label: str) -> bool:
    ...
    try:
        ...
        _LOGGER.info("Pressed %s for %s via homeassistant_voip_stack service", button_eid, device.get("name"))
        return True
    except Exception:
        ...
        async def wrapper(call: ServiceCall) -> None:
            ...
            if not device:
                raise ServiceValidationError(
                    f"homeassistant_voip_stack.{op_label}: no VoIP device matches the target"
                )
...
async def _handle_sip_account_create_service(call: ServiceCall) -> None:
    ...
    if not provided_password:
        persistent_notification.async_create(
            ...
            title="Home Assistant VoIP Stack SIP Account",
...
async def _async_register_services(hass: HomeAssistant) -> None:
    ...
    hass.services.async_register(DOMAIN, "sip_route", handle_sip_route, schema=sip_route_schema)
    hass.services.async_register(
        DOMAIN, "phonebook_add_contact", handle_phonebook_add_contact, schema=phonebook_add_schema
    )
    hass.services.async_register(
        DOMAIN, "phonebook_remove_contact", handle_phonebook_remove_contact, schema=phonebook_remove_schema
    )
    hass.services.async_register(
...
async def _async_setup_shared(hass: HomeAssistant, config: dict | None = None) -> None:
    ...
    _LOGGER.info("Home Assistant VoIP Stack loaded (SIP softphone + SIP B2BUA/router)")
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Set up Home Assistant VoIP Stack from a config entry (UI setup)."""
