...
const HA_SOFTPHONE_DEVICE_ID = "__homeassistant_voip_stack_ha_softphone__";
...
class VoipStackCard extends HTMLElement {
  ...
  set hass(hass) {
    ...
    if (hass) {
      ...
      // esphome_voip_stack last-reason entity, not from a HA bridge event.
  ...
  // esphome_voip_stack selects it by index, so the destination text shown by the
  // ESP equals location_name. Compare against this everywhere instead of the
  // hardcoded "Home Assistant" string literal.
  _getHaName() {
    return this._hass?.config?.location_name || "homeassistant-voip-stack";
  }
  ...
  async _loadAvailableDevices() {
    ...
    try {
      const result = await this._hass.connection.sendMessagePromise({
        type: "homeassistant_voip_stack/list_devices",
      });
      if (result?.devices) {
  ...
  _isVoipStackLoaded() {
    ...
    return !Array.isArray(components) || components.includes("homeassistant_voip_stack");
  }
  ...
  async _answer() {
    ...
    try {
      if (this._isHaSoftphoneMode()) {
        ...
        await this._hass.callService("homeassistant_voip_stack", "sip_answer", {
  ...
  async _decline() {
    ...
    try {
      if (this._isHaSoftphoneMode()) {
        await this._hass.callService("homeassistant_voip_stack", "sip_decline", {
          call_id: this._sessionCallId(),
          status: 603,
          reason: "Decline",
  ...
  async _hangup() {
    ...
    try {
      ...
      if (wasSoftphone) {
        await this._hass.callService("homeassistant_voip_stack", "sip_hangup", {
          call_id: this._sessionCallId(),
        });
      } else {
  ...
  async _toggleDnd() {
    ...
    try {
      const result = await this._hass.connection.sendMessagePromise({
        type: "homeassistant_voip_stack/set_ha_softphone_dnd",
        dnd: next,
      });
  ...
  async _loadSoftphoneState() {
    ...
    try {
      const result = await this._hass.connection.sendMessagePromise({
        type: "homeassistant_voip_stack/ha_softphone_state",
      });
  ...
  async _getDeviceInfo() {
    try {
      ...
      const result = await this._hass.connection.sendMessagePromise({
        type: "homeassistant_voip_stack/resolve_device",
