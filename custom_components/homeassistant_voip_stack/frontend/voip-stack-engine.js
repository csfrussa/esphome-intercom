...
const WS_AUDIO = 1;
const WS_SUBSCRIBE_CALL_EVENTS = "homeassistant_voip_stack/subscribe_call_events";
...
class VoipStackEngine extends EventTarget {
  ...
  async _wsUrl(deviceId) {
    ...
    const path = `/api/homeassistant_voip_stack/ws?device_id=${encodeURIComponent(deviceId)}`;
  ...
  async _setupAudio(deviceInfo, negotiated = null) {
    ...
    if (sendToEsp) {
      ...
      await this._audioContext.audioWorklet.addModule(`/homeassistant-voip-stack/voip-stack-processor.js?v=${encodeURIComponent(MODULE_VERSION)}`);
    ...
    if (receiveFromEsp) {
      await this._audioContext.audioWorklet.addModule(`/homeassistant-voip-stack/voip-stack-playback-processor.js?v=${encodeURIComponent(MODULE_VERSION)}`);
  ...
  async startHaSoftphone(target, softphoneInfo, context = {}) {
    ...
    const reply = await this._hass.callWS({
      type: "homeassistant_voip_stack/ha_softphone_start",
