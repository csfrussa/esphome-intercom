"""Optional Home Assistant Assist intent handlers for Intercom Native."""
...
INTENT_CALL = "IntercomCall"
INTENT_HANGUP = "IntercomHangup"
INTENT_ANSWER = "IntercomAnswer"
INTENT_DECLINE = "IntercomDecline"
...
async def _intercom_devices(hass: HomeAssistant) -> list[dict[str, Any]]:
    from .websocket_api import _get_intercom_devices

    return await _get_intercom_devices(hass)
...
async def _origin_device(intent_obj: Intent) -> dict[str, Any] | None:
    ...
    if not device_id:
        return None
    for device in await _intercom_devices(intent_obj.hass):
...
async def _phonebook_contacts(hass: HomeAssistant) -> list[str]:
    devices = await _intercom_devices(hass)
    contacts = [str(device.get("name") or "").strip() for device in devices]

    ha_phonebook = hass.states.get("sensor.intercom_phonebook")
    raw = ""
...
async def _resolve_area_contact(hass: HomeAssistant, raw_name: str) -> ContactResolution:
    """Resolve a spoken area name to one intercom device in that HA area."""
    ...
    devices = []
    for device in await _intercom_devices(hass):
...
class _IntercomIntentHandler(IntentHandler):
    """Base handler for local-satellite intercom commands."""
    ...
    async def _require_origin(self, intent_obj: Intent) -> dict[str, Any] | IntentResponse:
        ...
        if origin is None:
            return _response(intent_obj, "I do not know which intercom device heard that.")
        return origin


class IntercomCallIntentHandler(_IntercomIntentHandler):
    """Start an intercom call from the satellite that heard the command."""
    ...
    async def async_handle(self, intent_obj: Intent) -> IntentResponse:
        ...
        if resolved.error == "missing":
            return _response(intent_obj, "Which intercom contact should I call?")
        if resolved.error == "not_found":
            return _response(intent_obj, f"I cannot find an intercom contact named {spoken_target}.")
        if resolved.error == "ambiguous":
            return _response(intent_obj, f"The intercom contact {spoken_target} is ambiguous.")
        ...
        if resolved.error == "area_empty":
            return _response(intent_obj, f"The Home Assistant area {spoken_target} has no intercom device.")
        if resolved.error == "ambiguous_area_device":
            return _response(
                intent_obj,
                f"The Home Assistant area {spoken_target} has more than one intercom device.",
            )
        ...
        try:
            ...
            _LOGGER.error(
                "Assist intercom call failed: %s -> %s (spoken=%r): %s",
                origin.get("name"),
                resolved.canonical,
                spoken_target,
                err,
        ...
        _LOGGER.info(
            "Assist intercom call: %s -> %s (spoken=%r source=%s)",
            origin.get("name"),
            resolved.canonical,
            spoken_target,
            resolved.source,
...
class IntercomHangupIntentHandler(_IntercomIntentHandler):
    ...
    async def async_handle(self, intent_obj: Intent) -> IntentResponse:
        ...
        try:
            ...
            _LOGGER.error("Assist intercom hangup failed for %s: %s", origin.get("name"), err)
            return _response(intent_obj, "I could not hang up the intercom call.")
        return _response(intent_obj, "OK.")


class IntercomAnswerIntentHandler(_IntercomIntentHandler):
    ...
    async def async_handle(self, intent_obj: Intent) -> IntentResponse:
        ...
        try:
            ...
            _LOGGER.error("Assist intercom answer failed for %s: %s", origin.get("name"), err)
            return _response(intent_obj, "I could not answer the intercom call.")
        return _response(intent_obj, "Answering.")


class IntercomDeclineIntentHandler(_IntercomIntentHandler):
    ...
    async def async_handle(self, intent_obj: Intent) -> IntentResponse:
        ...
        try:
            ...
            _LOGGER.error("Assist intercom decline failed for %s: %s", origin.get("name"), err)
            return _response(intent_obj, "I could not decline the intercom call.")
...
def async_register_assist_intents(hass: HomeAssistant) -> None:
    ...
    for handler in (
        IntercomCallIntentHandler(),
        IntercomHangupIntentHandler(),
        IntercomAnswerIntentHandler(),
        IntercomDeclineIntentHandler(),
    ):
        intent.async_register(hass, handler)
    hass.data.setdefault(DOMAIN, {})["assist_intents_registered"] = True
    _LOGGER.info("Intercom Native Assist intents registered")
...
def async_unregister_assist_intents(hass: HomeAssistant) -> None:
    ...
    _LOGGER.info("Intercom Native Assist intents unregistered")
